---
name: yaswarm-healthcheck
description: "Host security hardening and risk-tolerance configuration for YaSwarm deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, YaSwarm cron scheduling for periodic checks, or version status checks on a machine running YaSwarm (laptop, workstation, Pi, VPS)."
---

# YaSwarm Host Hardening

## Overview

Assess and harden the host running YaSwarm, then align it to a user-defined risk tolerance without breaking access. Use YaSwarm security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim YaSwarm changes the host firewall, SSH, or OS updates; it does not.
...
5. YaSwarm gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if YaSwarm can access sensitive data.
...
### 2) Run YaSwarm security audits (read-only)

As part of the default read-only checks, run `yaswarm security audit --deep`. Only offer alternatives if the user requests them:

1. `yaswarm security audit` (faster, non-probing)
2. `yaswarm security audit --json` (structured output)

Offer to apply YaSwarm safe defaults (numbered):

1. `yaswarm security audit --fix`

Be explicit that `--fix` only tightens YaSwarm defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check YaSwarm version/update status (read-only)

As part of the default read-only checks, run `yaswarm update status`.
...
- Credential hygiene notes (location of YaSwarm creds, prefer disk encryption)
...
After YaSwarm install or first hardening pass, run at least one baseline audit and version check:

- `yaswarm security audit`
- `yaswarm security audit --deep`
- `yaswarm update status`

Ongoing monitoring is recommended. Use the YaSwarm cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `yaswarm cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `yaswarm update status`
...
Before creating, `yaswarm cron list` and match on exact `name`. If found, `yaswarm cron edit <id> ...`.
If not found, `yaswarm cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `yaswarm update status` (preferred for source checkouts and channels)
2. `npm view yaswarm version` (published npm version)

## YaSwarm command accuracy

Use only supported commands and flags:

- `yaswarm security audit [--deep] [--fix] [--json]`
- `yaswarm status` / `yaswarm status --deep`
- `yaswarm health --json`
- `yaswarm update status`
- `yaswarm cron add|list|runs|run`

Do not invent CLI flags or imply YaSwarm enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by YaSwarm compaction:
